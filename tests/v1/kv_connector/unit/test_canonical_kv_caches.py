...
def test_allocate_canonical_kv_caches():
    ...
    # -- single block tensor covering all positions
    ...
    assert bt.tensor.shape == (NUM_BLOCKS, full_page)
    assert bt.page_size_bytes == full_page

    # -- group_data_refs: 3 groups, each with 1 ref to the single tensor
    assert len(canonical.group_data_refs) == 3
    for gid, refs in enumerate(canonical.group_data_refs):
        group = config.kv_cache_groups[gid]
        num_layers = len(group.layer_names)
        assert len(refs) == 1
        assert refs[0].tensor_idx == 0
        assert refs[0].page_size_bytes == (
            group.kv_cache_spec.page_size_bytes * num_layers
        )
