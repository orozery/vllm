...
class KVConnectorModelRunnerMixin:
    ...
    def allocate_canonical_kv_caches(
        ...
        flat_buffer = torch.zeros(total_size, dtype=torch.int8, device=device)
        contiguous_buffer = flat_buffer.view(kv_cache_spec.dtype).view(
            physical_shape
        )
        ...
        # build kv_caches for attention kernels
        ...
        # Build a single flat (num_blocks, page_size) int8 block tensor
        # covering all positions.  The canonical physical layout places
        # all data for one block contiguously across every position
        # (including K and V), so one tensor suffices.
        block_tensor = flat_buffer.view(num_blocks, page_size)
        block_tensors = [
            KVCacheBlockTensor(tensor=block_tensor, page_size_bytes=page_size)
        ]

        # build group_data_refs — every group references the single
        # block tensor; page_size_bytes tells the connector how many
        # bytes of each block row belong to this group.
