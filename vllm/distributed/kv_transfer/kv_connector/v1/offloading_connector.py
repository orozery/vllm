...
from vllm.distributed.kv_transfer.kv_connector.v1.base import (
    KVConnectorMetadata, SupportsHMA
)
...
from vllm.v1.kv_cache_interface import (
    AttentionSpec, KVCacheConfig, MambaSpec, SlidingWindowSpec
)
...
class OffloadingConnectorScheduler:
    ...
    def __init__(self, spec: OffloadingSpec):
        ...
        # per group sliding window size in blocks, where None denotes full attention
        self.sliding_window_size_in_blocks: list[int | None] = []
        ...
        for idx, group in enumerate(spec.kv_cache_config.kv_cache_groups):
            ...
            if isinstance(kv_cache_spec, SlidingWindowSpec):
                ...
                sliding_window_groups.append(idx)
                self.sliding_window_size_in_blocks.append(cdiv(
                    kv_cache_spec.sliding_window, self.offloaded_block_size
                ))
            elif isinstance(kv_cache_spec, MambaSpec):
                # Mamba depends on a single state
                sliding_window_groups.append(idx)
                self.sliding_window_size_in_blocks.append(1)
            else:
                assert isinstance(kv_cache_spec, AttentionSpec)
                attention_groups.append(idx)
                self.sliding_window_size_in_blocks.append(None)

        # for lookups, we want to sort groups by their window size
        # attention groups first, then by decreasing window size
        sliding_window_groups.sort(
            key = lambda i: self.sliding_window_size_in_blocks[i], reverse=True
        )
        self.lookup_groups = attention_groups + sliding_window_groups
    ...
    def _update_block_hashes(self, req_status: RequestStatus):
        for group_idx, block_hashes in enumerate(req_status.block_hashes_groups):
            group_idx_bytes = group_idx.to_bytes(4, "big", signed=False)
    ...
    def _touch(self, req_status: RequestStatus):
        for group_idx, block_hashes in enumerate(req_status.block_hashes_groups):
            sw_blocks = self.sliding_window_size_in_blocks[group_idx]
            if sw_blocks is None:
                self.manager.touch(block_hashes)
            else:
                # we aim to keep just blocks that are necessary to hit
                # the original request (+ decoded blocks)
                blocks_to_skip = max(0, req_status.num_hit_blocks - sw_blocks)
                self.manager.touch(block_hashes[blocks_to_skip:])
    ...
    def _lookup(self, req_status: RequestStatus, start_block_idx: int) -> int | None:
        ...
        for group_idx in self.lookup_groups:
            ...
            if sw_blocks is None:
                # full attention
                hits = self.manager.maximal_prefix_lookup(block_hashes)
            else:
                # sliding window
                hits = self.manager.sliding_window_lookup(block_hashes, sw_blocks)
    ...
    def get_num_new_matched_tokens(
        ...
        if is_new_request:
            req_status.num_hit_blocks = start_block_idx + hits
    ...
    def update_state_after_alloc(
        ...
        for block_hashes, group_blocks, sw_blocks in zip(
            req_status.block_hashes_groups,
            block_groups,
            self.sliding_window_size_in_blocks,
        ):
            ...
            num_computed_gpu_blocks = next(
                (
                    i for i, block in enumerate(group_blocks)
            ...
            assert (
                ...
                <= cdiv(num_external_tokens, self.gpu_block_size)
            )
            if sw_blocks is not None:
                assert num_pending_gpu_blocks <= sw_blocks * self.block_size_factor
    ...
    def _get_reqs_to_store(
        ...
        for req_id, new_block_id_groups, _ in yield_req_data(scheduler_output):
            ...
            for block_hashes, block_ids, sw_blocks in zip(
                req_status.block_hashes_groups,
                req_status.block_id_groups,
                self.sliding_window_size_in_blocks,
            ):
                is_sliding_window = sw_blocks is not None
                for idx, blk_hash in enumerate(
                    ...
                    for i in range(self.block_size_factor):
                        ...
                        if is_sliding_window:
                            # TODO self.sw_blocks_being_stored[block_id] = job_id
                            pass
...
class OffloadingConnectorWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        for kv_cache_tensor in self.spec.kv_cache_config.kv_cache_tensors:
            tensor_layers = kv_cache_tensor.shared_by

            # verify a single attention backend per tensor
            assert (
                len({attn_backends[layer].get_name() for layer in tensor_layers}) == 1
            )

            # verify all mentioned layers indeed share the tensor
            assert len({kv_caches[layer].data_ptr() for layer in tensor_layers}) == 1

            # select one of the layers to represent the tensor
            layer = tensor_layers[0]
            kv_cache_tensors[layer] = kv_caches[layer]

        self._register_handlers(kv_cache_tensors, attn_backends)
